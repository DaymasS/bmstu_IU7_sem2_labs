...
argsIn="$fileDir../data/pos_01_args.txt"


while [ -f "$fileIn" ]; do
    if bash "$fileDir/pos_case.sh" "$fileIn" "$fileOut" "$argsIn"; then
    ...
    if [ "$numPosCases" -lt "10" ]; then
        ...
        argsIn="$fileDir../data/pos_0$numPosCases""_args.txt"
    else
...
argsIn="$fileDir../data/neg_01_args.txt"

while [ -f "$fileIn" ]; do
    if bash "$fileDir/neg_case.sh" "$fileIn" "$argsIn"; then
